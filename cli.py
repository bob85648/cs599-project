...
class AligoCLI:
    ...
    async def initialize_system(self):
        """初始化系统 - 使用懒加载优化启动速度"""
        if SYSTEM_CONFIG.get("enable_llm", True) and not LLM_CONFIG.get("api_key"):
            raise RuntimeError(
                "Missing LLM_API_KEY. Copy .env.example to .env or set the environment variable before starting."
            )
...
def run_health_check_standalone() -> int:
    ...
    if not LLM_CONFIG.get("api_key"):
        print("FAIL: missing LLM_API_KEY")
        return 1
